...
cooks_plot <- function(h, label, dist) {
  data <- data.frame(Observation = 1:length(dist), CookDistance = dist, Label = label)
  
  ggplot(data, aes(x = Observation, y = CookDistance)) +
    geom_segment(aes(xend = Observation, yend = 0), color = "navyblue") +
    geom_hline(yintercept = h, color = "darkred", linetype = "dashed") +
    geom_text(aes(label = Label), vjust = -0.5, color = "navyblue") +
...
cooks_distance <- output$influencers$`Cook's D`
dffits <- output$influencers$DFFITs
Hadi_influence <- output$influencers$`Hadi's Influence`
...
influence_guides <- data.frame(Observation = 1:length(cooks_distance), dist = cooks_distance, Label = cooks_label)
  
  cooksplot <- ggplot(influence_guides, aes(x = Observation, y = dist)) +
    geom_segment(aes(xend = Observation, yend = 0), color = "navyblue") +
    geom_hline(yintercept = cooks_cutoff, color = "darkred", linetype = "dashed") +
    geom_text(aes(label = Label), vjust = -0.5, color = "navyblue") +
    labs(title = "Cook's Distance", y = "Cook's Distance", x = "Observation") 
  
  cats <- list()
    cats$cooksplot <- ggplot(influence_guides, aes(x = Observation, y = dist)) +
    ...
    cooksplot + geom_hline(color = "darkred")
    
    cooksplot +
  theme(
    plot.background = element_rect(fill = "lightgray"),   # Background color of the entire plot
    panel.background = element_rect(fill = "white"),     # Background color of the plotting area
    axis.line = element_line(color = "black"),           # Color of the axis lines
    axis.text = element_text(color = "blue"),            # Color of axis text
    axis.title = element_text(color = "darkred"),        # Color of axis titles
    legend.background = element_rect(fill = "lightblue"),# Background color of legend
    legend.text = element_text(color = "purple"),        # Color of legend text
    legend.title = element_text(color = "darkgreen")     # Color of legend title
  )
...
influenceR <- function(model, data, plot = "none") {
  ...
  if (plot == "cooks" | plot == "all") {
    plots$influence_guides <- data.frame(Observation = 1:length(cooks_distance), 
                                   dist = cooks_distance, Label = cooks_label)
    
    plots$Cooks <- ggplot(influence_guides, aes(x = Observation, y = dist)) +
    geom_segment(aes(xend = Observation, yend = 0)) +
    geom_hline(yintercept = cooks_cutoff, linetype = "dashed", color = "darkred") +
    geom_text(aes(label = Label), vjust = -0.5) +
    labs(title = "Cook's Distance", y = "Cook's Distance", x = "Observation") 
