...
# Test 1: Input Validation - non-data.frame input
test_that("Input data must be a data.frame.", {
  model <- lm(mpg ~ wt + hp, data = mtcars)
  expect_error(influencer(model, matrix(mtcars), plot = "none"), 
               "Input data must be a data.frame.")
})

# Test 2: Input Validation - wrong model type
test_that("Input model must be an lm or glm object.", {
  expect_error(influencer(mtcars, mtcars, plot = "none"), 
               "Input model must be an lm or glm object.")
})

# Test 3: Input Validation - dataset is too small
test_that("Dataset is too small", {
  small_data <- mtcars[1:2, ]
  model <- lm(mpg ~ wt + hp, data = small_data)
  expect_error(influencer(model, small_data, plot = "none"), 
               "Dataset is too small")
})

# Test 4: Input Validation - too many NA values
test_that("Input data contains more than 5% NA values.", {
  data_with_na <- mtcars
  data_with_na[1:3, 1] <- NA  # Introduce some NA values
  model <- lm(mpg ~ wt + hp, data = data_with_na)
  expect_error(influencer(model, data_with_na, plot = "none"), 
               "Input data contains more than 5% NA values.")
})

# Test 5: Correct Output Structure
test_that("Output should be a data frame with correct columns", {
  model <- lm(mpg ~ wt + hp, data = mtcars)
  output <- influencer(model, mtcars, plot = "none")
  expect_true(is.data.frame(output))
  expect_equal(names(output), c("Cook's D", "DFFITs", "Hadi's Influence"))
})

# Test 6: Correctness of calculations (basic test)
test_that("Correctness of influencer calculations", {
  model <- lm(mpg ~ wt + hp, data = mtcars)
  output <- influencer(model, mtcars, plot = "none")
  
  # Assuming pre-calculated values for a specific row for testing
  expect_equal(round(output[1, "Cook's D"], 3), 0.046)
  expect_equal(round(output[1, "DFFITs"], 3), 0.195)
  expect_equal(round(output[1, "Hadi's Influence"], 3), 0.292)
})

# Test 7: Plotting functionality
test_that("Plotting does not throw errors", {
  model <- lm(mpg ~ wt + hp, data = mtcars)
  
  expect_silent(influencer(model, mtcars, plot = "none"))
  expect_silent(influencer(model, mtcars, plot = "cooks"))
  expect_silent(influencer(model, mtcars, plot = "DFFITS"))
  expect_silent(influencer(model, mtcars, plot = "Hadi"))
  expect_silent(influencer(model, mtcars, plot = "all"))
})
...
test_that("function influencer gives helpful errors", 
          {
            expect_error(influencer(model1, matrix(data1), plot = "none"), 
               "Input data must be a data.frame.")
...
## valid inputs
test_that("function influencer works with dataframes and lm/glm objects", {
